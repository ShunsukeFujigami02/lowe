  ...
  try
    {
      ...
      TH1D* hnoretroside = new TH1D("hnoretroside",Getchar("HISTTITLE"),(int)filemanagernoretroside->selectedfilemap.size(),Getdouble("HISTMIN"),Getdouble("HISTMAX"));
      for(int j = 0;j < (int)manager->listoffilelist["testnoretroside"].size();j++){
	double value = 0.;
	AnalizeFile file = manager->SetdoubleRun(value,Getchar("VALUERUN"),"number","testnoretroside",j);
	double xvalue = 0.;
	file.GetTagdouble(Getchar("XVALUE"),xvalue);
	hnoretroside->Fill(xvalue,value);
	delete h1;
	delete f1;
      }
      ...
      TH1D* hmeannoretrocenter = new TH1D("hmeannoretrocenter",";energy(MeV);mean of efficient hits direct part",11,4.5,15.5);
	...
	delete f1;
      ...
      TH1D* hmeanonretroside = new TH1D("hmeanonretroside",";energy(MeV);mean of efficient hits direct part",11,4.5,15.5);
	...
	double mean = 0.;
	double meanerror = 0.;
	AnalizeFile file = manager->SetTH1DEvent(h1,"efficienthitsOnretrodirect","number","testonretroside",j);
	double energy = 0.;
	file.GetTagdouble("ParticleEnergyFixedValue",energy);
	...
	delete f1;
      ...
      TH1D* hmeanonretrocenter = new TH1D("hmeanonretrocenter",";energy(MeV);mean of efficient hits direct part",11,4.5,15.5);
      for(int j = 0;j < (int)manager->listoffilelist["testonretrocenter"].size();j++){
	TH1D* h1 = new TH1D("h1","",100,0.,Getdouble("ONRETROHISTMAXMIN")+j*Getdouble("ONRETROHISTMAXGRAD"));
	double mean = 0.;
	double meanerror = 0.;
	AnalizeFile file = manager->SetTH1DEvent(h1,"efficienthitsOnretrodirect","number","testonretrocenter",j);
	double energy = 0.;
	file.GetTagdouble("ParticleEnergyFixedValue",energy);
	TF1* f1 = new TF1("f1","gaus",0.,Getdouble("ONRETROHISTMAXMIN")+j*Getdouble("ONRETROHISTMAXGRAD"));
	h1->Fit(f1,"Rn");
